...
describe('SpeechBubble', () => {
  ...
  it('thinking 模式显示三个跳动圆点', () => {
    const { container } = render(<SpeechBubble mode="thinking" />);
  ...
  it('cancelArmed 时显示取消提示（红色）', () => {
    render(<SpeechBubble mode="listening" cancelArmed={true} />);
    expect(screen.getByText('松开手指，取消发送')).toBeInTheDocument();
  });

  it('cancelArmed 时气泡背景为红色', () => {
    const { container } = render(<SpeechBubble mode="listening" cancelArmed={true} />);
