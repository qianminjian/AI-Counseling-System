...
- **存什么**：
  ...
  - `frozen/` — **冻结区**（真正冻结的远期任务，触发条件未到不实施、不合并，待后续开发时整合）：34 量表数字化/38 计费配额/39 画像量化与 AB 实验/40 水平扩展/41 信创适配/42 部署架构升级/43 家长端小程序化/58 转人工与安全模式/59 量表施测/60 商用合规备案/61 外部服务接入/62 研究数据脱敏/73 实体机器人/74 无屏交互终端（14 份，2026-08-08 D-16 对齐）
