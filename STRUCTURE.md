...
├── README.md               # 项目入口导航
│
├── design/                 # 设计层：BEACON.md + 总览/跟踪表 + 01~58 md 设计文档（单一事实源）+ frozen/（34/38-43/58 冻结区）
...
- **存什么**：
  ...
  - `DESIGN-OVERVIEW.md` — 58 份设计文档总览入口（目录导航 + 关键摘要）
  - `TASK-TRACKER.md` — 任务跟踪表（文档整合任务 + MVP 开发任务 + 决策/风险/里程碑）
  - `01_*.md` ~ `57_*.md` — **当前设计文档（md 格式，单一事实源）**，由 `doc/*.docx` 整合而来，直接平铺在 design/ 根下；16-33 为详细设计/功能/合规/测试；35-37/44-51 为 2026-07-28 解冻专题；52-55 为分析专题；56-57 为实施专题；58 在 frozen/
  - `frozen/` — **冻结区**（真正冻结的远期任务，触发条件未到不实施：34/38-43/58；35-37/44-51 已于 2026-07-28 解冻移回根目录）
