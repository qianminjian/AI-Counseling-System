...
| 编号 | 级别 | 描述 | 状态 |
|---|---|---|---|
| **BUG-SW-01** | P1 | workbox 导航 fetch 失败（`Failed to fetch` @ workbox）→ 页面重载/SW 接管 → **唤醒引擎每次重置"加载中"**（用户实测"叫了没反应"的元凶之一）；测试环境 unregister SW 后恢复正常，**代码未修复，待排查**（nginx/网络抖动 or workbox 配置） | OPEN 待排查 |
| **F-6 细化** | P1 | ① worker 内 transformers.js 探测后全量下载停滞（主线程手动下载可绕过）→ worker 60s 超时降级主线程；② 降级主线程推理识别后状态机卡"加载中"（缓存存在仍卡）；③ 首次 40MB 下载 3Mbps ~2 分钟（带宽硬约束） | OPEN 前端专项 |
...
### 加载链路优化（用户要求，commit 8a8ae59，已部署）
- **登录页挂载即预加载**（最早时机）：先 `preloadVoiceprintModel`（声纹，登录可用）→ 完成后 `preloadWakeModel`（唤醒，对话用）——顺序加载避免双路抢带宽
- **缓存利用**：模型走 transformers-cache（Cache API）+ nginx `immutable 1y`（HTTP 缓存）；缓存命中时登录页预加载秒完成，进对话即就绪
- EmotionSelect 保留唤醒 preload 兜底（单例复用）
