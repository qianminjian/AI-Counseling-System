...
## 第 2 轮（2026-08-11~12 全面遍历，UI-TEST-014）

**0 个功能 BUG**。观察项：

| 编号 | 等级 | 标题 | 状态 |
|------|------|------|------|
| OBS-P-03-01 | P2 | 情绪周报页缺少「近7天日期范围」「风险等级」「AI 建议」区块（design/12 §4.3 有原型） | OPEN |
| OBS-P-01-01 | P3 | 注册页无「确认密码」「协议勾选」（与 design/12 §4.2 模式 B 一致） | OPEN |
| OBS-P-02-01 | P3 | 登录页无「显示密码」「验证码登录入口」（模式 C 兼容通道无独立 UI） | OPEN |
| OBS-P-03-02 | P3 | 多孩切换不可验证（两账号均单孩绑定） | OPEN |

**本轮新增受限**：LIM-01 同意撤回实际提交 SKIP-数据保护（保护核心账号开心，未登记测试己/测试丁家庭码）；LIM-02~04 P-07 扫码向导/绑定校验/已绑定分流仅代码层证据（UAT 无设备数据）。

> 详情见 reports/browser-test/UI-TEST-014-parent.md（本轮）