...
- **修复 commit**：`761fefc`（SettingsPanel 三态文案 + ChatRoom 传 wakeAuthorized）
- 状态：FIXED | 复测结果：**VERIFIED ✅（2026-08-12）**——未授权时显示「语音唤醒已开启（未授权麦克风）」；已授权显示原文案
...
- **修复 commit**：`761fefc`（登录页底部双链接 + 4 章节弹层）
- 状态：FIXED | 复测结果：**VERIFIED ✅（2026-08-12，curl 证实 JS 已上线；旧浏览器需 SW 更新后可见）**
