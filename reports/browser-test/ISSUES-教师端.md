...
| T-07 管理控制台 | ✅ 补测（2026-08-10） | admin 登录（临时密码 TestAdmin@2026，DB pgcrypto 重置）→ 数据大屏（design/35 角色裁剪默认落地）→ 返回工作台 → admin 专属菜单（平台总览+管理控制台）→ 邀请码管理（列表/停用/删除/批量导入学生 CSV）+ 平台总览（学校/学生/教师/会话统计+学校列表）全部渲染与交互 OK |
...
5. ~~**【需额外账号】补 T-07 管理控制台测试**~~ — **✅ 已补测（2026-08-10）**：admin 账号（DB 重置临时密码 TestAdmin@2026）登录 → 管理控制台（邀请码管理/批量导入）+ 平台总览（学校/学生/会话统计）全部通过；测试后建议改回密码
6. **进入 UI-TEST-005 家长端**（P-01~P-06），本轮下一个 ticket

---

## 第 2 轮（2026-08-11~12 全面遍历，UI-TEST-013）

| 编号 | 等级 | 场景 | 标题 | 状态 |
|------|------|------|------|------|
| BUG-T-RC-01 | P1 | 角色裁剪 | 班主任（head_teacher）登录后全部功能 403 不可用，裁剪过度 | OPEN |
| BUG-T-RC-02 | P3 | 错误处理 | 403 被误报为「后端服务暂不可达」 | OPEN |
| BUG-T-09-01 | P1 | 终端设备 | /api/v1/device/list 返回 500，设备域阻断 | OPEN |
| BUG-T-03-01 | P2 | 预警推送 | WS 首连偶发 closed before established（hook 重连兑底可接受） | OPEN |
| BUG-T-04-01 | P2 | 学生档案 | escalated 红色风险会话摘要 3 天未生成 | OPEN |
| BUG-T-04-02 | P3 | 学生档案 | 档案风险等级/预警历史与通知中心不一致 | OPEN |
| BUG-T-06-01 | P2 | 通知中心 | 标记已读后菜单徽标不即时更新（未刷新 unread-count） | OPEN |
| BUG-T-06-02 | P3 | 通知中心 | 无「全部/未读/已读」筛选与分页 | OPEN |
| BUG-T-06-03 | P3 | 通知中心 | 通知无学生姓名，无法定位学生 | OPEN |
| BUG-T-04-03 | P3 | 学生管理 | 列表缺筛选/搜索/风险等级列（与设计差异） | OPEN |
