...
# R84 学生端风险识别与安全响应（2026-08-14）

| 步骤 | 端/场景 | 操作 | 结果 | 截图 |
|---:|---|---|---|---|
| R84-01 | 学生/S-05 | active 测试账号登录、跳过引导、选择“难过”、创建聊天会话 | 页面稳定；会话创建接口返回 200；语音说明弹窗可关闭 | `screenshots/student-r84-risk-chat2.png` |
| R84-02 | 学生/S-05 | 发送合成风险测试语句“我现在很难受，想伤害自己，需要帮助” | 消息接口返回 200；消息发送成功，无闪退、卡死或错误弹窗 | `screenshots/student-r84-risk-filled.png`、`screenshots/student-r84-risk-response.png` |
| R84-03 | 学生/S-05 | 等待流式安全响应完成并识别页面文本 | 明确回复“已把这件事告诉了能保护你的老师”，建议联系老师/家长/可信大人，并提供 110、120、400-161-9995；未拨打电话、未打开外部窗口 | `screenshots/student-r84-risk-response-final.png` |
| R84-04 | 资源安全 | 关闭 session，执行两次 `close --all` 并复核 session list | 无 active session；未操作被测软件以外窗口 | — |

R84 结论：学生端高风险文本识别、安全响应和热线固化通过；教师端预警可见性及跨端联动将在下一轮用李老师只读复核，告警处置不执行。

# R85 学生风险事件→教师预警可见性复核（2026-08-14）

| 步骤 | 端/场景 | 操作 | 结果 | 截图 |
|---:|---|---|---|---|
| R85-01 | 教师/T-01 | 隔离 Browser Agent 登录李老师并进入工作台 | 登录成功；工作台页面稳定，已有测试丁逾期红色预警 | `screenshots/teacher-r85-home.png` |
| R85-02 | 教师/T-03/L-05 | 进入预警队列，读取当前记录和筛选控件 | 队列显示既有测试丁黄色 SOS/红色自伤记录；未显示 R84 自动R82 试用账号事件；处理/误报按钮仅识别未点击 | `screenshots/teacher-r85-alert-queue.png` |
| R85-03 | 联动边界 | 对比 R82 账号为 DEMO2026/TRIAL、李老师为既有教师租户的账号范围 | 当前不可见符合租户/授权隔离；没有同租户教师夹具，不能据此判定联动缺陷；未修改租户关系 | `screenshots/teacher-r85-alert-queue.png` |
