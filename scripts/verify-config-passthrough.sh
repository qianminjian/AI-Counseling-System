...
SERVICE_VARS=(
  # OPS-016（doing/95）：补 REDIS_HOST/PORT/PASSWORD——doing/87 RUNTIME-002 覆盖键读取器（_read_override）
  # 消费 redis 连接三变量，此前漏登记致契约检查 6 项 FAIL（既有漂移）
  "tts-service:DASHSCOPE_API_KEY,DASHSCOPE_TTS_MODEL,TTS_SYNTHESIZE_TIMEOUT,TTS_CORS_ORIGINS,REDIS_HOST,REDIS_PORT,REDIS_PASSWORD"
  "voice-service:ASR_ENGINE,SER_ENABLED,DASHSCOPE_API_KEY,VOICE_PROCESS_TIMEOUT,VOICE_ANALYZE_TIMEOUT,VOICE_CORS_ORIGINS,REDIS_HOST,REDIS_PORT,REDIS_PASSWORD"
)
