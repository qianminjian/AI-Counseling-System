...
- 模型：`onnx-community/whisper-tiny`（多语言含中文，量化后约 20MB），首次启用从 HF 国内镜像（hf-mirror.com）下载一次，之后走浏览器 Cache API 缓存；真机唤醒率不理想可升级 whisper-base（约 45MB，中文更好）。
...
4. **授权弹窗措辞**（新增语音唤醒单独授权 + 修正现有"本地处理"诚实性表述——合规措辞由用户审定）；
5. **唤醒词定稿**（推荐"哈喽波波"，可后续加"你好波波"变体）。
