...
> ⚠️ **大文档读取纪律（2026-08-13）**：本文件 239KB / 1155 行，**禁止全量 Read**。请按下表行号范围用 `start_line/end_line` 分段读取。行号随编辑漂移，定位前先 `grep -nE '^#{1,3} '` 校准。

## 📑 分段索引（按行号定位）

| 章节 | 行号 | 内容 |
|------|------|------|
| 头部更新登记块 | 1-20 | 历次合并/审计登记（**超长单行，非必要不读**） |
| §一 文档整合 | 49-86 | 当前阶段整合任务 |
| §二 待确认决策 | 87-100 | **待拍板决策（高频查阅）** |
| §三~五 MVP/M2/商业化 | 101-157 | 已完成历史 |
| §六 风险与问题 | 158-170 | 风险跟踪 |
| §七 里程碑 | 171-183 | 里程碑计划 |
| §二十三 设计驱动开发总表 | 595-667 | **按优先级 P0/P1/P2/远期（高频）** |
| §二十四 设计深化追踪 | 740-815 | 逐篇 triage |
| §二十七 过度设计待议 | 857-887 | 议决项 |
| §二十八 深度审计回填 | 888-969 | 回填任务 |
| §二十九~三十五 ticket | 970-1155 | UI测试/告警/Admin/无屏/toC/M3/审计 ticket |

> **常用速查**：待决策→§二(87行)；当前 backlog→§二十三(595行)；挂账 ticket→§二十九起(970行)。

---

> 创建：2026-07-23 | 更新：2026-08-09
> **doing/95 深度审计问题清单合并归档（DOC-121，2026-08-12）**：3 路独立 agent 审计 + P0 交叉复验——OPS-001/002（voice 双 P0）、BACK-001（教师端 classScope 越权面 5 入口治理）、BACK-002（短信警示）、FE-001（DEMO2026 移除）等 P0×5 + P1×6 + P2×13 + P3×8 全闭环（含 code-review H-1/M-1/L-1）；全量回归后端 2190 + 前端 1385（942+214+192+37）+ Python 50 + shell 15/15 全绿；最终态并入 03/04/05/06/09/11/12/15；文件归档 his/95；doing 区清空。
> 创建：2026-07-23 | 更新：2026-08-12
> **板块05 剩余P1+高价值P2清扫批次执行登记（F7，2026-08-12）**：P1-4 随访调度登记挂账（裁决5 落定：checkFollowupDue/getDueFollowups/shouldReferToCase/shouldTerminateRetest/CaseSummary 5 方法保留不动，产品确认上线时再接，本期不接线）/**P1-5 AdminService 导入半成品闭环**（ImportResult 增 initPasswords 第4组件分发随机6位初始密码 + parseCsvLine 引号感知解析 RFC4180 基础（项目无 CSV 库依赖，自写单行解析）+ classCode 必填/超长预校验；AdminController 透传 initPasswords；AdminServiceTest 8 用例 + AdminControllerTest 同步）/ **P2 清扫 10 项**：①板块01 OpsController.requireConfirm 自递归死代码删除（grep 全仓零消费）②板块05 P2-1 maskPhone 三处重复收敛 counseling-common PhoneMasker（前3后4，PiiDesensitizer.maskPhone 私有且后2规则不可复用故新建单源）③P2-2 AliyunSmsService HttpClient 设 requestTimeout 10s（connect+read 双超时）④P2-3 PhoneVerificationService 先发短信成功再写 Redis（发送失败不占冷却键，验证码语义与冷却时长不变）⑤P2-4 TeacherNote 解密 5 处收敛 TeacherNoteStore.decryptContent 读单点（S-006 readDecryptedMessages 模式，加密侧不变）⑥P2-5 DataAnalyticsService 风险频率统计统一 detectedAt 口径（4 处 createdAt→detectedAt）⑦P2-7 OpsInsightsService channelStats 改 DB GROUP BY 下推（tenant_template.notifications）+ DurationBetween 重命名 durationBetween⑧板块04 P2-1 PromptVersionService.invalidateCache 注释改述诚实描述 keys 实现（prompt:* 键规模有限，阻塞风险可接受，低风险方案）⑨板块04 P2-4 RiskMetricsJob.countOverdue SQL 下推 SLA 四档 OR 条件（selectCount，首条 eq+lt 直连避空 or() 非法 SQL）⑩板块06 P2-1 RiskEvent.fromDetection/CounselingSession.create 魔法值改引既有常量/补常量 + P2-2 EmotionDiaryMapper upsertCheckin 租户一致性纵深防线测试（断言 tenant_id 与上下文一致）；**backlog 登记**：其余 P2 项（含 ConversationServiceImpl 依赖 21 重构候选、AdminConsole P1/P3 等）未实施，留后续批次清理；本批次未动 design/doing 报告、未做 git 写操作
> **doing/98 设计与代码全量深度审计登记（2026-08-13）**：3 路独立只读 agent 并行审计（后端/前端/Python+部署+设计一致性）——39 项发现（P0×0 / P1×7 / P2×11 / P3×21），综合评分 6.2、发布就绪 6.5；核心问题域=代码逻辑虚化度（假实现/死表/契约假象/半接线）；实施批次：P1 全闭环（D-01 告警 sum() 修正 / B-01 删 ProfileEffectivenessTracker（PROF-024 回退未实现待 frozen/39 重建）/ B-02 删 VoicePersonaMatcher（TMATCH-002 职责由 VoiceDegradationPolicy 承担）/ B-03 声纹 enroll 语义修复（后端侧闭环：UPLOADED 停留、enroll+taskId 驱动 COMPLETED、DeviceManagement 轮询 5min 超时兑底；真实 enroll 由设备固件携带 taskId 调用——固件侧对接仍登记 frozen/73 跟踪，故审计报告 §八「声纹 enroll ❌ 半接线」表述与后端侧修复状态并存，以后端侧闭环为准） / F-01 student-h5 表外端点入表+消费面双向扫描 / F-16 parent-h5 fillPath 接线+apiPath 助手 / F-02 parent 契约多文件扫描）+ P2 台账修正（D-02 BEACON 17 规则/阈值单值 / D-03 prepare-models.yml 注释 / D-04 design04 §5.5 / D-06 STRUCTURE.md / D-07 ci.yml 注释 / F-05 删 fetchSosTools+设计回改 / F-06 SOS 安全计划静态引导裁决）+ P3 清理（D-05 ser_engines 越界保护 / D-08 tts Dockerfile 对齐 / D-10 dashscope 启动期鉴权探测 / B-07 SecurityConfig headers 合并 / B-08 @Transactional 移除 / B-09 log.warn / F-07/F-08 错误处理收敛+loading 死状态 / F-11~F-17 前端杂项 / F-13 vite-plugin-pwa 死依赖 / F-03/F-04 类型补强）；**B-04 提级登记**：device/report 通道无 HMAC 签名（frozen/73 §九 固件侧未就绪）——生产已上线运行态风险，按「生产已上线运行态缺陷最高优先级」原则**提级 P1 跟踪**（后端单方面加校验会拒绝真实设备上报，需固件配合，不因冻结登记降级）；**B-05 加密待办登记**：FieldEncryptionService 已接线但 ENCRYPTION_ENABLED 默认 false，生产 PII 字段（消息摘要/教师备注/学生画像）明文落库——商业化前必须启用 + 存量回填（frozen/60 COMP-008 关联，未成年心理健康数据 PIPL 合规风险）；剩余 P2×1（B-06 预合成矩阵已随 B-02 收敛）+ P3×若干登记待后续批次；报告落 design/doing/98_设计与代码全量深度审计报告_20260813.md（分块底稿 tmp/audit98/）。
> **doing/98 合并归档（DOC-123，2026-08-13）**：39 项全部闭环（补实施 F-09 admin-web 12 端点显式 VO + F-10 错误模型统一 shared ApiError + D-09 仓库级 .githooks/commit-msg 可选强制；B-05 加密=评估闭环待商业化前启用、B-10 量表=冻结区记录、F-15/F-18=登记型、B-04=裁决提级登记、B-03 固件侧=frozen/73）——最终态并入 04 §9.3（告警语义 D-01/D-10）/ 05 §8.6.1（消费面双向扫描）/ 06 §5.2.1（dashscope 探测）/ 08（声纹 enroll 语义）/ 09 §10.5~10.6（SOS 裁决）/ 10 §2.10·§8.1（PROF-024/TMATCH-002 回退）；文件归档 his/98；doing 区清空；全量回归后端 mvn verify + Python 108/56 + 前端 958/220/219/49 + shell 15 套件全绿（admin-web 49 测试首次全量纳入）。
> **今日变更深度审计登记（DOC-126，2026-08-14）**：improve-codebase-architecture 深度审计 2026-08-14 全部 126 提交（BFS 工具链 + 三项遗留闭环等）——8 候选（Strong×2：99-1 BFS 进程级全量重放模型 / 99-5 DeviceSecurityService 双装配缝；Worth×3：99-2 BFS cli 接口面 / 99-6 设备端点签名样板 / 99-7 加密字段注册表；Speculative×3：99-3 EmotionSelect 预加载删除 vs BEACON #25 红线一致性 / 99-4 useSseStream 超时命名 / 99-8 恒等透传 seam）全部待议决；候选清单落 design/doing/99_今日变更深度审计_20260814.md；Top：99-1 优先（34 分钟 7 连修实证）
> **doing/99 今日变更深度审计合并归档（DOC-127，2026-08-29）**：8 候选全部议决闭环——99-1/99-2 BFS 引擎/CLI 适配解耦（399e04eb：bfs-engine+cli-adapter 分文件 + 150 行单测 + JSON 通道分离 + 超时双检测）+ 99-3 BEACON #25 冲突闭环（27726422 误删裁决，e02c7e7b 恢复情绪页预加载）+ 99-4 SSE 超时 CONNECT/IDLE 语义拆分（5c18bba1，行为零变化）+ 99-5/6/7 装配缝收敛/@DeviceSignature 声明式/EncryptedFieldRegistry 单一事实源（c1e332e9，含 check-commit 构造器纪律钩子，后端 582 用例全绿）+ 99-8 裁决不实施（2026-08-29 项目负责人：99-7 注册表已覆盖加密感知需求，恒等透传保持配置驱动接口契约）；最终态并入 03 §ARCH-005（SSE 超时语义拆分）/ 06 §3.5（加密字段单一事实源 + 99-8 裁决）/ 15 §二（BFS 遍历引擎）/ BEACON #25 附注（误删-恢复溯源）；文件归档 his/99；doing 区清空。
> **三项遗留待办实施完成登记（DOC-125，2026-08-14）**：① **B-04 设备上报 HMAC 签名（P1）后端侧完成**——DeviceSecurityService.verifyRequestSignature（HMAC-SHA256(body|ts|nonce) + ±5min 时间戳窗口 + 常量时间比对）+ reportOnline 下发 deviceSecret（固件签名密钥）+ 渐进式开关 MINDSAFE_DEVICE_SIGNATURE_MODE（OFF 默认/LOG 暴露未签名设备/ENFORCE 强制）+ heartbeat/status/voiceprint 三端点接线；40 用例全绿；固件侧 NST-HW-02 二期对接后置 ENFORCE 闭环（frozen/73 §九 同步）；② **B-05 生产加密启用就绪（P1 合规）**——V49 加密列容量迁移（toc_family_accounts.phone→96 / teacher_notes.content→TEXT，含 rollback）+ EncryptionBackfillRunner 启动自动回填存量明文（幂等四字段：message_summaries.content_summary / counseling_sessions.session_summary / teacher_notes.content / toc_family_accounts.phone，5 用例）+ scripts/gen-encryption-key.sh 密钥生成脚本 + .env.example/compose 透传；生产启用动作仍归 frozen/60 COMP-008 商业化门禁（置 true+配密钥重启即自动回填）；③ **DB-ROLLBACK-01（P2）闭环**——V39~V48 共 10 个迁移 down 脚本补齐（rollback/ 22 个）+ V49 自带 rollback，db-rollback-drill.sh check 配对 ✓，04 §5.5 违约记录改闭环；验证：后端全量回归 BUILD SUCCESS（service + api 全绿）
> **全文档代码一致性台账对齐登记（DOC-124，2026-08-13）**：3 路独立只读 agent 盘点 14 份主文档 + 本表 + 测试报告资产 vs 代码实态——54 项失实（A 级 27/B 级 26/C 级 11）全部回灌（详见 BEACON 2026-08-13 行）；**新增待办**：① **DB-ROLLBACK-01（P2）**：V39~V48 共 10 个迁移无 down 脚本，违约 04 §5.5 E-4「V34+ 新迁移必须带 down 脚本」承诺——补写 rollback/ 脚本 + 演练记录（回滚现由 restore.sh 备份恢复兜 底）；② **UI-TEST-009/010**：模板示例 ticket 未排期（A 系列已随 UI-TEST-015 落地，标记不再单独执行）；③ **UI-TEST-011~017**：报告已产出补登记（见 §二十九）；④  本机 target 目录 jacoco 陈旧残留报告（11.0%）勿作引用，覆盖率以 CI 报告为准。
...
| TOC-GATE-001 | **toC 认证验证码回显上线门禁登记（裁决 8：门禁登记 + 标注，不改短信通道）** | ⚠️ 试点期演示行为登记（板块09 P1-1，2026-08-12 专题 H） | 演示行为：试点期 SMS=logging（冻结决策）下 send-toc-code 响应回显验证码（`TocSendCodeResult.code`，任意手机号可注册登录 → 档案 CRUD/隐私删除，属红线域演示认证）；前端类型已标 `@deprecated` + TODO[GATE-TOC-001] 门禁锚点（parent-h5/src/services/toc.ts）；**移除条件：接入短信通道（.env 设 SMS_PROVIDER=aliyun）即移除回显**——届时删除 code 字段与回显消费（toc-login setEchoCode）并改造 tocFlow.e2e.test.ts 为 mock 短信通道断言；后端 send-code 实现本次不动（裁决 8） |
...
| PROF-024 | 画像效果回收（有/无画像会话质量对比接 39/45）+ 无效维度降权自校准 + 教师侧脱敏摘要与订正回流 | P2 近期偏后 | design/46 P2 | PROF-020 | ~~✅ 已完成（2026-07-28）~~ → ❌ 已删除（doing/98 B-01：零调用假实现，2026-08-13，待 frozen/39 解冻重建） |
...
| TMATCH-002 | 画像匹配微调+手动偏好记忆回写46 + 安全/危机稳定基调锁定+预合成矩阵（统一 TTSFX-002）+ 三方同源接线 | P1 近期 | design/48 P1 | UX-005/PROD-003 | ~~✅ 已完成（2026-07-28）~~ → ❌ 已删除（doing/98 B-02：注入零调用死代码，职责由 VoiceDegradationPolicy/VoicePersonaResolver 承担，2026-08-13） |
...
> 52. **DOC-118 登记**（2026-08-12）：doing/92 第五轮架构深化候选清单合并归档——R-001~029 全部议决闭环（19 实施 + 3 议决不实施 R-012/018/029 + 2 冻结移交 frozen/92/93 R-006/027 + 2 排期前端批次 R-001/R-003 + 1 部分 R-004）；最终态并入 03 §5.3（TokenType/parseOnce/TTL 缓存/schema 常量）+ 09 §3.4（SAFE-202 召回语义改追加更正 + reviewJson 落库 + 撤回删除范围清单 + R-007 重新授权）；git mv 至 his/92，doing 区清空（仅 .gitkeep）。
> 52. **DOC-116 登记**（2026-08-11）：doing/90 轻量安全扫描报告合并归档——git mv 至 his/90（第三文档，同号异题）；完成度核验：19 项扫描全 ✅（P0/P1 0 项）+ P3 建议实施（8ee9b505）+ 改进建议核验闭环（070a2bc0）；doing 区剩 87/88/89/91/92。
> 51. **DOC-115 登记**（2026-08-11）：第五轮架构深化候选清单（improve-codebase-architecture，排除前四轮已审计范围，直接生成 md）——doing/92 新建（R-001~029：student-h5 API 无端点登记【Strong】/consent 撤回后无法重新授权【Strong 合规】/业务日界服务器时区【Strong】/JwtTokenProvider 前缀剥离 4 处重复【Strong 近三天事故根因】等 29 项；删除测试 10 项含 ChartCard 已删除证实/01_schema.sql 过期 16 版/DEMO2026 后门）；首要建议 R-007→R-010→R-016。
> 50. **DOC-114 登记**（2026-08-11）：第四轮架构深化候选清单（improve-codebase-architecture，排除前四轮已审计范围）——doing/91 新建（Q-001~010：敏感类别双套【P0】/忽略名单三天三连追加【P0】/平台表 schema 边界【P0 独立批次】/降级话术数据面污染/同步辅助无超时/错误码面碎片/迁移序列一致性/指标口径 N-004 关联/CI 分层/日志脱敏双处）；首要建议 Q-002→Q-001→Q-004。
> 49. **DOC-113 登记**（2026-08-11）：doing/90 合并归档——双文档 git mv 至 his/90（第三轮架构深化候选清单 + 前端缺陷与 IP 解析 SPEC）；doing/90 10/10 处理完毕（9 实施 + P-003 冻结移交 frozen/90）后按 §4 合并；doing 区剩 87/88/89。
> 48. **DOC-112 登记**（2026-08-11）：P-003 声纹 pgvector 冻结移交独立专题——doing/90 P-003 ❄️（项目负责人指令：不在本专题处理），新建 frozen/90 声纹向量化独立专题（根因：生产数据回填 + 阈值语义调优 + 写入链路改造，独立批次）；doing/90 全部处理完毕（9 实施 + 1 冻结移交），满足合并条件待确认。
> 47. **DOC-111 登记**（2026-08-11）：第三轮架构深化候选清单（improve-codebase-architecture，排除前三轮已审计范围）——doing/90 双文档：①候选清单（P-001~010：IP 解析两套语义/四空壳域/声纹 pgvector/Prompt 缓存/LoginPage 真实 UI bug×2/nginx 声纹限流/metrics_common 复制/tenant 漂移/等级映射重复/小域策略）；②前端缺陷与 IP 解析修复 SPEC（P-005+P-001 首要批次：ClientIpResolver 最右语义 + LoginPage 反引号 + PIN 上限 6，AC-90-01~06）；接线状态更正（CaseLifecycle/VoiceEmotion/TrendAnomaly/Entitlement/MoodCheck 均已接线）。
> 46. **DOC-110 登记**（2026-08-11）：非 device 域架构深化候选清单（improve-codebase-architecture 第二轮，排除 device/toc 域）——doing/89 双文档：①候选清单（N-001~013：认证域碎片化/唤醒词 23 次热点/家长域 withdrawn 旁路/统计口径三处/摘要域租户红线/会话 23 参构造/教师域 980 行/OpsController 22 端点/画像域接缝/记忆域评分/前端语音 DSP/面板样板/部署审计）；②首要建议 SPEC（N-001 认证域 + N-003 家长域 withdrawn 旁路：LoginRateLimiter 接口 + AuthProvider 接缝 + ParentIdentityResolver 统一解析 + WeeklyReportService 下沉，AC-89-01~08）；与 frozen/89 同号异题。
> 45. **DOC-109 登记**（2026-08-11）：无屏终端域架构深化候选清单（improve-codebase-architecture 聚焦近三天变更）——doing/88 新建（AD-001~009：设备档案浅门面深化/上报安全通道接线/租户名单规则源/绑定归属校验/配置面授权分裂/声纹状态机原子化/Controller 适配层统一/前端双客户端/管理端路由注册表）；走查中修复 4c64ad9 编译回归（84e78df）；首要建议 AD-002→AD-004 合并批次（设备身份与归属校验模块）；design/13 领域词汇表 device 域术语待补。
> 44. **DOC-108 登记**（2026-08-11）：74/84/85 三专题合并设计归档（项目负责人指令：合并设计文档 + 更新测试指导/自动化测试方案 + 回归测试）——最终态并入 01 §六（无屏交互终端：A/B/C 三分 + 配置状态机 + 关键资产）、08 §六（无屏终端配置体系：6 页面 + 端点 + V39/V43 + AC-84）、12 §5（toC 家庭版：能力落地表 + 页面 + 验收）；三文件归档 his/74、his/84、his/85；doing 区仅剩 87；05/15 测试文档同步更新；DESIGN-OVERVIEW v6.24。
...
> 34. **DOC-095 登记**（2026-08-09）：doing/83 双文档合并归档（后台管理端 AdminConsole + 服务降级监控与告警，同号异题双文件）——最终态按主题并入主文档：**01** §六 平台管理后台 AdminConsole（十二能力域 M1~M12 状态表 + DEC-007 关键决策 + P0~P3 四期实施路线，原后续章节编号顺延）；**03** §8 监控运维（规则数更新 8→17 + 指标表补降级/业务指标 + §8.1 服务降级监控链路：3 降级规则契约/检测器 OPS-MON-007/采集器 OPS-MON-008/通知链/部署契约）、§5.2 平台表豁免（IGNORE_TABLES 恒忽略名单 8 表）、§5.3.1 平台管理账号体系（platform_admin + PLATFORM_ token + 四角色 + 防爆破）；**02** §6.6 平台级表（V34~V38：degradation_events/alert_events/platform_admin/service_health_snapshots/sys_config(+V38 种子)/sys_config_history/sla_escalation_log/usage_events + prompt_versions.status 列）；**07** §2.8.1 计量计费三层设计（usage_events 采集层先行已实现，rate_plans/subscriptions/billing 冻结待 frozen/38）。过程数据（to-spec/to-tickets/SPEC AC/ticket 表）不入主文档（跟踪在 TASK-TRACKER §三十/§三十一）；OPS-MON-006 状态 ✅；两文件归档 his/83（文件名可区分）；DESIGN-OVERVIEW v6.14、BEACON 演进日志同步。
> 34b. **DOC-095 修正登记（2026-08-10，追加修正不改历史结论）**：DOC-095 提前归档撤销——ADMIN tickets（§三十一 29 个）未实施完毕即归档，与 design-document-management §4「开发完成时合并」字面不符（项目负责人确认：主文档只承载最终完整版本，不记录过程）——① his/83 双文档回迁 design/doing/（实施态承接返回 doing/83，ADMIN tickets 跟踪不变）；② design/14 主文档撤销归档 his/14（DOC-098 的 14 号承接职责返回 doing/83）；③ design/01 §六 设计态回滚（章节编号顺延恢复，头部标注修正）；④ DESIGN-OVERVIEW v6.19 同步；后续归档纪律：以实施完成（ticket 全 ✅）为前提，不得提前归档。
> 33. **DOC-093 登记**（2026-08-09）：doing/84 toC 归属声明 + 待办——84 覆盖 toB+toC 统一配置体系（二合一），**终端侧通用配置**（扫码入口/配网页/设备绑定/配网流程）toB/toC 共用不受冻结影响；**toC 专属配置项**（家长端 H5 自助配置页等）归属 frozen/85（toC 家庭版，已冻结），待解冻后实施；**待办：84 逐项 toB/toC 标注工作后续纳入计划**（实施 doing/84 时同步处理，归属声明已写入 84 头部决策记录后）。
> 32. **DOC-092 登记**（2026-08-09）：doing/74 无屏交互终端多形态方案内容归属分类治理（项目负责人指令：分析哪些本项目没要开发、抽出子专题、其余非本项目开发转冻结）——**A 类（本项目 toB 软件要开发，保留 doing/74）**：§5 技术架构软件侧、§8.1~8.5 后端适配（终端设备管理 API/通信适配/无屏声纹编排/管理台模块/私有化对接）、§4.2~4.5 软件侧机制、§11 阶段 1 软件任务（配置面细化已在 doing/84）；**B 类（本项目没要开发，抽出子专题）**：toC 家庭版产品模式（74 §1.4 扩展二/§1.5/§8.6/§9 toC 维度/§14 中 toC）→ **新建 frozen/85 波波小伙伴 toC 家庭版_方案与SPEC（已冻结）**（10 个 TOC ticket TOC-001~010 待解冻立项 + toC-AC-1~10 验收标准；**2026-08-09 项目负责人指令：B 类一并先冻结**，解冻触发=toC 立项决策，关联 frozen/60 商用合规/frozen/43 家长端小程序化）；**C 类（非本项目开发，转冻结）**：硬件/固件/3D 打印/外壳/采购/波波 IP（74 §3.1 固件侧/§6 多形态实现/§7 嵌入式输入/§10 硬件方案/§9.1 硬件维度）→ 冻结归口 doc/NST-HW-02 波波蛋硬件落地实施方案 + frozen/73 实体机器人（互补）；74 头部新增「内容归属分类」表 + 相关章节标注（B 类抽至 doing/85/C 类冻结）。
...
| UI-TEST-002 | 环境与数据准备（test compose 起服 + init-school.sh + 账号矩阵 + 测试数据基线 + 静态基 线断言） | 环境 | ✅ 完成（UAT 环境账号矩阵/数据基线就绪） |
| UI-TEST-003 | 学生端遍历 S-01~10 + 问题登记 + 修复-部署-复测闭环至收敛 | 学生端 | ✅ 收敛（2026-08-10）：10 场景 9✅+1⚠️；3 问题全 VERIFIED（PIN 提示/PIN 6 位/a11y label） |
| UI-TEST-004 | 教师端遍历 T-01~08 + 问题登记 + 修复-部署-复测闭环至收敛 | 教师端 | ✅ 收敛（2026-08-10）：4✅+3⚠️+1⏭；P0（WS 推送）与 P2（低分率）VERIFIED；T-07 admin 专项登记受限项 |
| UI-TEST-005 | 家长端遍历 P-01~06 + 问题登记 + 修复-部署-复测闭环至收敛 | 家长端 | ✅ 收敛（2026-08-10）：3✅+3⚠️→复测 4/4 VERIFIED（410 语义/consent-status/文案）；数据污染已处理 |
| UI-TEST-006 | 联动场景 L-01~06 + 问题登记 + 修复-部署-复测闭环至收敛 | 三端联动 | ✅ 收敛（2026-08-09）：L-01~05 ✅ + L-06 ⚠️ 部分；6 项修复链（TENANT/LLM/UI/KB/AUDIT）闭环 |
| UI-TEST-007 | 汇总报告（reports/browser-test/SUMMARY.md）+ doing/82 合并归档（DOC-085） | 全量 | ✅ 汇总完成（2026-08-10）；归档随 DOC-087 |
| UI-TEST-008 | 对话窗口专项执行（C-01~07：假麦克风注入 + 服务级 ASR/TTS 真链路 + 唤醒/记忆/暖场/超时 全链路）→ ISSUES-对话专项.md → 修复-部署-复测循环 | 语音/对话 | ✅ 收敛（2026-08-09/10）：C-01~06 ✅/⚠️ + C-07 代码层 VERIFIED（单测）；音频注入受限项登记（C-04/C-07 真机补测） |
| UI-TEST-009 | 管理端遍历 A-01~A-17 + 修复闭环（design/15 §十 模板示例） | 管理端 | ⬜ 未排期（2026-08-13 台账修正：A 系列已随 UI-TEST-015 落地，本 ticket 不再单独执行） |
| UI-TEST-010 | 四端第二轮回归（design/15 §十 模板示例） | 全量 | ⬜ 未排期（2026-08-13 台账修正：实际执行拆分为 011~017 专项） |
| UI-TEST-011 | 学生端语音引擎专项（F-19~F-27 修复后：预加载/引擎就绪/唤醒近音/转写循环/冷却关窗） | 语音 | ✅ 执行完成（2026-08-10，报告 reports/browser-test/UI-TEST-011-voice-engine.md；2026-08-13 台账修正：原无 ticket 记录，报告已产出未登记） |
| UI-TEST-012 | 学生端第二轮回归 | 学生端 | ✅ 执行完成（2026-08-10，UI-TEST-012-student.md） |
| UI-TEST-013 | 教师端第二轮回归 | 教师端 | ✅ 执行完成（2026-08-10，UI-TEST-013-teacher.md） |
| UI-TEST-014 | 家长端第二轮回归 | 家长端 | ✅ 执行完成（2026-08-10，UI-TEST-014-parent.md） |
| UI-TEST-015 | 管理端遍历 A-01~A-17 + 修复闭环 | 管理端 | ✅ 执行完成（2026-08-10，UI-TEST-015-admin.md；A 系列全部落地） |
| UI-TEST-016 | 复测轮（修复后回归） | 全量 | ✅ 执行完成（2026-08-10，UI-TEST-016-retest.md） |
| UI-TEST-017 | 遗留修复验证 | 全量 | ✅ 执行完成（2026-08-12，UI-TEST-017-遗留修复.md） |
...
| ADMIN-P2-04 | M10 通知渠道统计 + 失败台账 + 触达策略 | 后端 | ✅ 实施完成（2026-08-09，渠道统计+脱敏台账+ChannelPage；触达策略配置化经 YAGNI 议决关闭——当前预警链路=站内+WS+企微告警，无多渠道分发点，改造待真实多通道需求） |
| ADMIN-P2-05 | M12 运营洞察 | 后端 | ✅ 实施完成（2026-08-09，质量/漏斗/健康度，3 用例） |
| ADMIN-P2-06 | 前端 P2 页面组（四页） | 前端 | ✅ 实施完成（2026-08-09，降级矩阵/知识库/通知渠道/运营洞察 4 页） |
| ADMIN-P2-07 | P2 回归门禁（P2-01~06 全绿） | 全量 | ✅ 实施完成（2026-08-09，后端 BUILD SUCCESS + 前端 9 用例 + tsc OK；记录型切换语义） |
| ADMIN-P3-01 | usage_events 采集层（M4 先行，幂等） | 后端 | ✅ 实施完成（2026-08-09，V37 + UsageCollector 双任务） |
| ADMIN-P3-02 | 用量报表（计量预览标注） | 前端 | ✅ 实施完成（2026-08-09，UsagePage + 合规页 CompliancePage；finance/audit 只读放行 H2 修复） |
| ADMIN-P3-03 | M11 合规视图 | 后端 | ✅ 实施完成（2026-08-09，/compliance/consent-stats） |
| ADMIN-P3-04 | P3 回归门禁（P3-01~03 全绿，冻结项不实施） | 全量 | ✅ 实施完成（2026-08-09，后端 BUILD SUCCESS + 前端 9 用例；计费/导出审批冻结未实施） |

---

## 三十二、无屏交互终端配置体系 ticket（2026-08-09，DOC-091 登记）

> 登记说明：doing/84 无屏交互终端配置体系（方案与 SPEC）的 CFG ticket **本表为唯一执行跟踪表**（状态/排期/归属）；AC 定义（验收标准断言 AC-84-01~28）见设计文档 §四~§八（唯一 AC 定义处，DRY）。跨专题依赖：CFG-008 → doing/83 M13 挂载点（ADMIN 系列排期）；CFG-011 → frozen/43 W-1 企业主体认证；CFG-012 → NST-HW-02 §3.6 二期。frontier：P1-B（CFG-008 admin-web M13 + CFG-009）待 doing/83 排期。

| Ticket | 任务 | 阶段 | 归属 | 状态 |
|--------|------|------|------|------|
| CFG-001 | device 域 4 表 + 设备上报/心跳端点（§六） | P0 | 后端 | ✅ 已实施（2026-08-09，4d39665，V39 + DeviceService/Controller） |
| CFG-002 | 扫码入口页（parent-h5 `/p/:v/:deviceCode` + 离线兜底 + 步骤条） | P0 | 前端 | ✅ 已实施（2026-08-09，4d39665） |
| CFG-003 | 配网引导层 + 回连检查轮询（复用原生 captive portal） | P0 | 前端 | ✅ 已实施（2026-08-09，4d39665） |
...
| CFG-011 | 小程序通道（scene 短码，前置 43 号 W-1） | P2 | 前端 | ❄️ 冻结移交（2026-08-11 DOC-105 归口 frozen/43 W-1，当前专题不再跟踪） |
...
## 三十三、波波小伙伴 toC 家庭版 ticket（2026-08-10，DOC-100 解冻登记）

> 登记说明：doing/85 波波小伙伴 toC 家庭版（方案与 SPEC）的 TOC ticket **本表为唯一执行跟踪表**（状态/排期/归属）；AC 定义（验收标准断言 toC-AC-1~10）见设计文档 §五（唯一 AC 定义处，DRY）。跨专题依赖：TOC-003 → doing/84 设备管理 API（CFG-001~008 已交付）；TOC-008 → 家长端/老师端统一框架；TOC-009 → 立项评估（商业模式 + frozen/60 商用合规 + frozen/43 小程序化）。frontier：TOC-001（账号体系）可启动，前置 85 号立项决策确认。
...
| TOC-004 | 成长报告 toC 模板 | TOC-001 | ❄️ 冻结移交（2026-08-11 DOC-105 归口 frozen/89，解冻=toC 对话链路立项；当前专题不再跟踪） | ⬜ 待排期 |
| TOC-005 | 预警推送 toC 通道 | TOC-001 | ❄️ 冻结移交（2026-08-11 DOC-105 归口 frozen/89，同上） | ⬜ 待排期 |
| TOC-006 | 远程管理（音量/灯光/音色/偏好 + 设备状态） | TOC-003 | ✅ 软件侧已实施（c6bb2cc）；灯光/固件执行 ❄️ 冻结移交（DOC-105 归口 frozen/73 §九） | ✅ 软件侧完成 |
...
| Ticket | 任务 | 归属 | 状态 |
|--------|------|------|------|
| AUDIT-DEEP-001 | 设备 token 过期校验 + 签名改造（P1-01：内嵌 exp 24h + HMAC/JWT + 单测） | counseling-service | ✅ feed6c8 |
| AUDIT-DEEP-002 | reportOnline serverUrl 加固（P1-02：首次注册禁匿名重写 + 签名更新 + pullConfig 认证） | counseling-service | ✅ feed6c8 |
| AUDIT-DEEP-003 | sys_config 运行时消费接线（P2-01：1-2 个 HOT 项真实消费） | counseling-service | ✅ feed6c8 |
| AUDIT-DEEP-004 | IGNORE_TABLES 文档同步（P2-02：03 §5.2 16 表 + 分类） | 文档 | ✅ feed6c8 |
| AUDIT-DEEP-005 | 降级矩阵注释统一（P2-03："记录型声明 + 覆盖键预留"） | counseling-service | ✅ feed6c8 |
| AUDIT-DEEP-006 | admin-web 页数台账修正（P2-04：25→17） | 文档 | ✅ feed6c8 |
| AUDIT-DEEP-007 | design/14 补建或改记（P2-05：DOC-098 引用悬空修复） | 文档 | ✅ feed6c8 |
| AUDIT-DEEP-008 | Prometheus 查询编码统一（P3-01：UriComponentsBuilder） | counseling-service | ✅ feed6c8 |
| AUDIT-DEEP-009 | TtsController engine 透传（P3-02：/health 字段对齐） | counseling-api | ✅ feed6c8 |
| AUDIT-DEEP-010 | language_mode 僵尸字段清理（P3-03） | tts-service | ✅ feed6c8 |
| AUDIT-DEEP-011 | 公开端点限流（P3-04：voiceprint/verify + device 端点） | counseling-api | ✅ feed6c8 |

> 执行顺序：001/002（P1 安全）→ 003 → 004~007（台账修复）→ 008~011；每批完成即回归。
> **全部闭环（2026-08-11，commit feed6c8 + 6951971）**：code-review 追加 P0-1（reportOnline 重签绕过）/P1-1（未绑定 serverUrl 改写）/P2-1（非法值回落）/P2-2（注释失实）/P3-1~P3-4 一并修复；回归 service 1093 + api 443 + ai 520 + tts 95 + voice 40 全绿；本专题已合并归档（DOC-111，his/88）。
...
- 审计/专题文档未全闭环一律不得写入 `design/his/`；必须先落 `design/doing/`，闭环后经项目负责人确认合并再归档
- 审计遗留问题（裁决/backlog/待核）必须在 TASK-TRACKER 有登记，禁止散落无跟踪
