...
| UI-TEST-008 | 语音专项遗留登记（SKIP-语音 清单移交 prod compose 手动专项） | 语音 | ⬜ 待排期 |

> 执行顺序：UI-TEST-002 → 003 → 004 → 005 → 006 → 007（UI-TEST-008 与 007 并行）；依赖：003~006 各自完成后立即触发修复-部署-复测闭环（3 轮上限），L 场景依赖 S/T/P 收敛后环境。
