...
> **P0 专题 G 冻结决策实施一致性收敛（DOC-118，2026-08-12）**：全部闭环——G-P0-1 S-009 风险事件统一入口迁移补全（LongTermMemoryService/OutputSafetyReporterImpl/SosEventService 收口 RiskEventWriter，needsNotify 语义不变：RED/SOS/召回升级 true、YELLOW 留痕/memory 信号 false；ConversationRiskProcessor/SessionEndAnalyticsService 死字段清理）/ G-P0-2 知识库查重口径 status IN ('active','published')（V30 published 兼容）+ 幂等测试 / G-P0-3 时区回归收敛 CounselingTimeZone（PlatformService 去 systemDefault、DataAnalyticsService 删 ZONE_CN）/ G-03P2-5 BEACON #23 remote 阈值标注（OD-014 全链 0.70）；**裁决 1-B 登记**（D-7 两级摘要保真：实现采纳 ORANGE riskLevel≥2 为最终口径，design/02 §7.2.1 + design/08 §2.5 追加裁决登记，不改决策）；BEACON #27 ③ numThreads=2 同步；design/08 §2.9 补 HEAD_TEACHER；后端局部回归全绿；**板块04 审计裁决 3 执行登记（P1-3，2026-08-12）**：PromptEvalGovernance 去留——裁决前提「无消费方」经 grep 全仓核验不成立：AdminPromptController（counseling-api）构造注入 + `POST /api/v1/admin/prompts/rollout-eval` 端点（PEVAL-004 灰度放量评估，fix-gate 已接线：safetyMean/evalDelta 从库读数）真实消费 evaluateRollout，AdminPromptControllerTest 3 用例验证端点行为；审计报告 04 P1-3 断言「grep 全仓仅定义处 4 匹配、无 main 调用方」失实（实测 25 匹配，main 代码消费 4 处：构造注入 + 端点调用）；**不删除**，照 DOC-064 台账修正模式登记说明（不改审计报告历史结论）；PEVAL P2 灰度流程经 /rollout-eval 已达生产入口，与 PEVAL-004 登记（:674）一致；**板块04 审计裁决 4 执行登记（P1-5，2026-08-12）**：KnowledgeMetadata 年级判定删除——matchesGrade/isSearchableForGrade 经 grep 全仓核验 main 零消费（仅 ReviewWorkflowTest 自用断言，已同步删除）；类保留（record 构造器被 KnowledgeBaseController/EditorialWorkflowService 消费）；年级判定单一实现落 RagAdvisorService.matchesGradeBand（正文 grade_band 正则，RagAdvisorServiceTest 覆盖），KB-102 过渡态双套判定收尾；**板块04 审计裁决 6 执行登记（P1-7，2026-08-12）**：Redis 会话状态缺失时 DB 兜底重建——sendMessageStream/endSession 经 CounselingSessionStore.findOwned（SEC-001 租户+学生+会话三重归属条件）重建 SessionState 并写回 Redis，DB 不持久化 emotionTag 重建态按默认情绪推进；不重建不调 LLM 的拒绝路径测试覆盖（RedisFallbackRebuild 4 用例）；归属语义未因 Redis 缺失削弱；**板块04 P1 批次执行登记（P1-1/2/4/6/8，2026-08-12）**：P1-1 SEC-001 会话归属负面测试 4 用例（跨租户/跨学生 sendMessageStream/endSession/updateClientSettings/sendNudgeStream 均断言 FORBIDDEN）+ design/09:2257 台账五触点修正；P1-8 SummaryCompensationJob 直连 CounselingSessionMapper 收敛 CounselingSessionStore.findSummaryCompensationCandidates（BA-11，三终态扫描+SCAN_LIMIT 上限+risk≥2 保真/提炼双分支+幂等不重补）；P1-4 SLA_DISPOSE_MINUTES 收敛 risk 包 RiskSlaConstants（3→15/2→60/1→480/0→1440，null/未知回落 1440）+ RiskOverviewService 硬编码 ZoneId.of(Asia/Shanghai) 改 CounselingTimeZone；P1-6 MessageSummaryService UTF-16 substring → TextUtils.truncateByCodePoint（code point 截断，domain MessageSummarySummarizer 同源共用，含 emoji 代理对测试）；P1-2 ConversationServiceImpl 依赖 24→21——删 fieldEncryptionService 死字段（零消费，MessageSummaryService 自持加密）+ 抽 RoundContextAssembler 组件（S-002 上下文组装单点，带走 longTermMemoryService/allianceEnhancer/contextAgent 3 依赖）；目标 ≤16 未达（剩余 21 依赖共享度高：sessionStore/sessionStateStore/messageSummaryService/riskProcessor/promptOrchestrationService 等为核心共享，流式组装/消息持久化/生命周期收尾抽离均需跨组件传回共享依赖，收益低于成本，列 P2 重构候选）；全量回归 160 用例全绿
