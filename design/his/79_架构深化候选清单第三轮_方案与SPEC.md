...
> 状态：✅ 已实施（2026-08-08，全部 23 候选闭环；实施记录见 §32）；剩余 BA-06 随 frozen/38 议决（唯一待议决项）；DA-04/DA-05 已议决实施（批次 E）
...
## §32 实施记录（2026-08-08，/implement 批次A/B/C/D/E）

> 范围：23 候选全部闭环（批次A/B/C + 批次D 收尾 FA-06/DA-02/DA-03 + 批次E 议决 DA-04/DA-05）；BA-06 随 frozen/38 议决（唯一待议决项，不阻塞清单闭环）。全部闭环后并入主文档并归档 his。
...
| 待议决 | BA-06 EntitlementFilter | ⏳ | 随 frozen/38 议决（映射对齐或冻结 删 filter） |
...
### 32.2 批次 E（2026-08-08，/implement 议决：DA-04/DA-05）

> 议决过程：方案 A（CI 全栈 e2e）因 8983862a 历史移除（不可持续）+ 契约层已被 *IT.java/openapi 快照覆盖而否决；**选定方案 B（deploy 现场后置冒烟）**——服务器具备完整真实环境（DB/真实 LLM/TTS），正是 smoke-test.sh 设计用途（design/04 §部署后冒烟）；deploy.sh 已有 ssh/retry + service-manager 健康检查基建，追加成本低。
