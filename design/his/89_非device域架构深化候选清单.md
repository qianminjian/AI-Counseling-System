...
> 状态：**已合并归档（2026-08-11，DOC-114，his/89）**——N-001~013 全部议决闭环（N-001/003 SPEC 批次 DOC-112 + N-004~011 实施 da0f44b + N-005 评估闭环 + N-006/012/013 议决不实施） | 与 frozen/89 toC 数据链路依赖跟踪**同号异题**（文件名可区分）
...
| N-004 | ✅ 已闭环（2026-08-11） | negativeLabels 单源常量 + 时区统一 CounselingTimeZone.SHANGHAI（WeCom/Usage/Ops 3 处收编） |
| N-005 | ✅ 评估闭环（2026-08-11） | runAsSystem 13 处一致消费 + dispatchInsights 已显式——接缝已存在，加抽象违背 YAGNI |
| N-006 | ⬜ 议决不实施（2026-08-11） | 会话 740 行重构风险大（文档自标注），收益/风险比不足，保留现状 |
| N-007 | ✅ 已闭环（2026-08-11） | AlertTodoMutePolicy/CaseLifecycleService 反哺 Spring 注入（替换内联 new）+ 9 测试适配 |
| N-008 | ✅ 已闭环（2026-08-11） | requireConfirm() 收编 5 处同构 if（危险操作 X-Confirm 统一校验） |
| N-009 | ✅ 已闭环（2026-08-11） | 头注释修正（提炼链路显式化）+ LIMIT 20 分页接缝标注 |
| N-010 | ✅ 已闭环（2026-08-11） | 评分单次化（filter/sort 2 次/条 → 缓存 1 次/条，ScoredMemory record） |
| N-011 | ✅ 已闭环（2026-08-11） | audio-utils 共享模块（rms/downsample 收编，useVoiceprint/useWakeWord 只引用） |
| N-012 | ⬜ 议决不实施（2026-08-11） | Speculative 候选，usePanelData 抽象收益未验证（YAGNI） |
| N-013 | ⬜ 议决不实施（2026-08-11） | Speculative 候选，deploy-lib.sh 已是全仓最佳分层范例，收益低 |
