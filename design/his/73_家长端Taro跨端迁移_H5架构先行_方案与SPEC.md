...
> 创建：2026-08-07 | 状态：**已完成（2026-08-07，T0~T5 全验收 ✅）** | 对应专题：frozen/43（家长端小程序化设计）、design/30 §方向五（PARENT-WX-001~006）
>
> ⚠️ **归档注记（2026-08-07，DOC-070）**：本专题实施完成，最终态已并入主文档（design/12 适用对象/最新口径/分阶段表/框架表 4 处 + design/08 §4.1 家长端技术形态行 + frozen/43 实态注记 + his/26 修正说明追加），本文归档 `design/his/73` 只读溯源，内容不再更新。
>
> 定位：frozen/43 判定家长端（Vite + React H5 SPA）技术栈不符合后续演进规划（微信小程序渠道），需 Taro 跨端迁移。本文按项目负责人指令「**先对现有 H5 进行技术架构迁移，输出仍保持 H5 单页应用**」输出**两阶段路线**：**P0 = H5 架构迁移（本方案主体）**——把 parent-h5 原地改造为 Taro 工程，H5 产物行为/URL/部署完全等价；**P1 = 微信小程序编译与认证接入（后续专题，依赖企业主体）**。本文只做设计，不含实现代码。
...
| 2026-08-07 | **T4 测试对齐完成**：coverage exclude 修正（Taro CLI 配置 config/**、babel.config.cjs 与声明文件 app.config.ts 不计入）；阈值门禁 50/50/30/50 → **80/80/80/80**（AC-12）；verify 补 6 用例（注册模式/密码校验/注册成功/失败路径/loading）、consent 补 5 用例（守卫/无年级/无 data/非 Error/默认文案）；**实测 117/117 全绿，覆盖率 statements 100 / branches 93.56 / functions 94.33 / lines 100，四维度 ≥80% 达标** | ✅ |
| 2026-08-07 | **T5 验证完成**：dev 冒烟——端口 5174 无残留 → 启动 dev server → 浏览器 6 项验证全过（登录页/注册模式切换/PIPL 页/未登录守卫重定向×2/已登录 report+consent 渲染与 API 错误兜底）；补装 dev 依赖 @pmmmwh/react-refresh-webpack-plugin + react-refresh（Taro dev 模式 Fast Refresh 必需，生产构建不需要）；**AC-11 产物体积：迁移前基线 256K → 迁移后 331 KiB（+29.3% < 30% 阈值，达标）**；CI 注释同步（vitest.config.ts）；README 技术栈行同步（parent-h5 Taro 4 + React 18）；DEPLOY-GUIDE/deploy.sh 零改动确认（build 脚本已切 taro CLI、dist 结构等价、nginx /parent/ 指向不变） | ✅ |
