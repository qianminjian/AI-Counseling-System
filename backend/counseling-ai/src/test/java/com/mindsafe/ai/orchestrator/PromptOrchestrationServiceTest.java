...
class PromptOrchestrationServiceTest {
    ...
    private OrchestrationContext ctx(int grade, int effectiveGrade, String entryMood,
        ...
        // ARCH-004：向后兼容 6 参构造已删除，改用生产 10 参完整构造
        return new OrchestrationContext(grade, effectiveGrade, entryMood, currentEmotion, riskLevel,
                null, EmotionState.STABLE, 0, false, false);
    }

    /** ARCH-004：resolve() 死路径已删除，测试改走 resolveWithTransition().profile() */
    private StrategyProfile resolve(OrchestrationContext ctx) {
        return service.resolveWithTransition(ctx).profile();
    }
    ...
    class SafetyOverride {
        ...
        void 橙色风险_安全锁定_危机策略_禁CBT() {
            StrategyProfile p = resolve(ctx(5, 3, "happy", null, RiskLevel.ORANGE));
        ...
        void 安全话术不降级_用真实年级() {
            ...
            StrategyProfile p = resolve(ctx(5, 3, "sad", null, RiskLevel.RED));
        ...
        void 黄色风险_不触发安全锁定_走正常情绪策略() {
            StrategyProfile p = resolve(ctx(3, 3, "sad", null, RiskLevel.YELLOW));
        ...
        @Test
        void 无风险信号_null不炸() {
            StrategyProfile p = resolve(ctx(3, 3, "calm", null, null));
            assertThat(p.safetyLocked()).isFalse();
    ...
    class EmotionGate {
        ...
        @Test
        void 情绪激活态_门控关闭CBT() {
            StrategyProfile p = resolve(ctx(4, 4, "anxious", null, RiskLevel.GREEN));
            assertThat(p.emotionState()).isEqualTo(EmotionState.ACTIVATED);
            assertThat(p.allowCbt()).isFalse();
        ...
        @Test
        void 稳定态_放行CBT() {
            StrategyProfile p = resolve(ctx(4, 4, "happy", null, RiskLevel.GREEN));
        ...
        void 学生端标签scared_归一为fearful策略() {
            StrategyProfile p = resolve(ctx(2, 2, "scared", null, null));
    ...
    class EmotionSourceAndGrade {
        ...
        void 轮级currentEmotion优先于会话级entryMood() {
            ...
            StrategyProfile p = resolve(ctx(4, 4, "happy", "sad", null));
        ...
        void currentEmotion空白_回退entryMood() {
            StrategyProfile p = resolve(ctx(4, 4, "angry", "  ", null));
            assertThat(p.entryMood()).isEqualTo("angry");
        }

        @Test
        void 年级降级透传_镜映按有效年级取材() {
            StrategyProfile p = resolve(ctx(5, 2, "anxious", null, null));
    ...
    class TemplateVariables {
        ...
        @Test
        void 八个变量齐全且非空() {
            StrategyProfile p = resolve(ctx(3, 3, "sad", null, null));
        ...
        void 门控关闭_cbt_gate为不进入认知重构() {
            StrategyProfile p = resolve(ctx(3, 3, "anxious", null, null));
    ...
    class ProfileTuning {
        ...
        private OrchestrationContext ctxWithSignals(String entryMood, RiskLevel riskLevel, ProfileSignals signals) {
            ...
            return new OrchestrationContext(4, 4, entryMood, null, riskLevel, signals,
                    EmotionState.STABLE, 0, false, false);
        }

        @Test
        void 高内向高置信_稳定态_改留白低压开场放慢节奏() {
            ProfileSignals signals = new ProfileSignals(0.8, 0.6, List.of(), 0.0);
            StrategyProfile p = resolve(ctxWithSignals("happy", null, signals));
        ...
        void 激活态_情绪策略优先_内向不覆盖开场() {
            ...
            StrategyProfile p = resolve(ctxWithSignals("anxious", null, signals));
            assertThat(p.opening()).isEqualTo(OpeningStrategy.STABILIZE_FIRST);
        }
        ...
        void 兴趣可用_镜映取材拼入提示() {
            ...
            StrategyProfile p = resolve(ctxWithSignals("sad", null, signals));
        ...
        void 兴趣低置信_不拼入取材() {
            ...
            StrategyProfile p = resolve(ctxWithSignals("calm", null, signals));
        ...
        void 合规裁决短路_画像微调全部失效() {
            ...
            StrategyProfile p = resolve(ctxWithSignals("happy", RiskLevel.ORANGE, signals));
            assertThat(p.safetyLocked()).isTrue();
            assertThat(p.opening()).isEqualTo(OpeningStrategy.STABILIZE_FIRST);
            assertThat(p.emotionMirrorHint()).doesNotContain("恐龙");
        }
