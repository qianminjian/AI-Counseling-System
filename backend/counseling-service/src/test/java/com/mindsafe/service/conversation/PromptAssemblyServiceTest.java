 ...
 * （SYS_001 → LANG_xxx → EMO_001 → CBT 指令 → RAG 固定顺序，空段省略），
...
class PromptAssemblyServiceTest {
    ...
    class MainPromptTests {
        ...
        @Test
        @DisplayName("SYS_001 → LANG_001 → EMO_001 → CBT 指令 → RAG 全段拼接（1-2 年级）")
        void assembleMain_fullChain() {
            ...
            PromptAssemblyService.AssembledPrompt prompt = service.assembleMainPrompt(TENANT_ID, STUDENT_ID, 2, "开心",
                    Map.of("strategy", "s1"), mockStageMark(), "【RAG】");
        ...
        @Test
        @DisplayName("RAG 为空 → 不追加 RAG 段，其余段照常")
        ...
        void assembleMain_sysVariables() {
            ...
            service.assembleMainPrompt(TENANT_ID, STUDENT_ID, 4, null, Map.of(), mockStageMark(), "");
        ...
        @Test
        @DisplayName("年级分层边界：5-6 年级 → LANG_003 + grade_level=5-6")
        void assembleMain_grade56() {
            ...
            service.assembleMainPrompt(TENANT_ID, STUDENT_ID, 6, "开心", Map.of(), mockStageMark(), "");
    ...
    class NudgePromptTests {
        ...
        @Test
        @DisplayName("SYS_001 → LANG_001 → TSK_004 三段拼接（1-2 年级）")
        void assembleNudge_order() {
            ...
            String prompt = service.assembleNudgePrompt(TENANT_ID, STUDENT_ID, 2, "平静",
                    Map.of("silence_seconds", "30", "warmth_level", "1", "direction", "问候"));
