...
/**
 * UsageTimeLimitService 每日使用时长限制单测（AUTH-030）：累加/查询/上限判定/剩余配额，
...
class UsageTimeLimitServiceTest {
    ...
    void setUp() {
        ...
        service = new UsageTimeLimitService(redisTemplate, 30, meterRegistry);
    }
    ...
    void isExceeded_disabled() {
        service = new UsageTimeLimitService(redisTemplate, 0, meterRegistry);
        assertThat(service.isExceeded(tenantId, userId)).isFalse();
    }
