...
# doing/87 RUNTIME-001 + 板块10 P2-1（doing/97）：覆盖键读取器收编 py-common 共享模块
...
def _read_tts_override():
    """读覆盖键（P2-1：委托 py-common 共享实现，fail-open 语义不变）"""
    return _read_shared_override("tts", log=logger.warning)
